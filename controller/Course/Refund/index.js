...
const { validateUpdateRefund, validateCreateRefund } = require("../../../joiSchemas/Course/Refund");
...
const { joinRefundRoomAndEmitMessage, sendNotifications } = require("../../../socket/helpers");
...
async function generateUniqueCode() {
    ...
    while (!unique) {
        ...
        const existingCode = await courseRefundModel.findOne({ where: { ticketNumber: newCode } });
        if (!existingCode) {
            unique = true;
        }
    }
...
const includeAttribute = {
    include: [
        ...
        { model: courseModel, include: [{ model: userModel, attributes: ["email", "firstName", "lastName", "profilePic"] }] },
...
const createRefund = async (req, res) => {
    ...
    try {
        ...
        const chkPrevRefund = await courseRefundModel.findOne({
            where: {
        ...
        const ordered = new Date(order.dataValues.createdAt);
        if (Math.floor((new Date() - ordered) / (1000 * 60 * 60 * 24)) > 7) {
            return res.status(400).send(responseObject("Refund request failed", 400, "", "Refund request failed because Order is more than 7 days old"))
        }

        const courseId = order.courseId;
        const course = await courseModel.findByPk(courseId);
        if (!course) return res.status(400).send(responseObject("Course Not Found", 400, "", "Id is not valid"));
        ...
        const refund = await courseRefundModel.create({
            orderId: order.orderId,
        ...
        const data = await courseRefundModel.findByPk(refund.refundId, {
            ...includeAttribute
        });

        //////////////////////////////////////////////////////////////////////////////////////
        let authorEmail = await courseModel.findByPk(courseId, {
            attributes: ['authorEmail']
        });
        authorEmail = authorEmail.dataValues.authorEmail;
        //
        const chat = await refundChatsModel.create({refundTicket: ticketNumber}, { transaction: t });
        //
        let receiverEmails;
        let chatUsers = [adminEmail];
        if (adminEmail == authorEmail) { // if Admin is Seller
            receiverEmails = [adminEmail];
            await refundUsersChatsModel.bulkCreate([
        ...
        await t.commit();
        //
        const chatData = await refundChatsModel.findOne({
...
const getARefund = async (req, res) => {
    try {
        ...
        if (user.role === "admin") {
            ...
            if (refund.course.user.email === userEmail) {
                return res.status(200).send(responseObject("Successfully Reterieved", 200, refund));
